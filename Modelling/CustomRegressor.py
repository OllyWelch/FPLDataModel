...
class CustomRegressor(BaseEstimator):
    ...
    def _resolve_estimator(func_name: str):
        ...
        funcs = {'linear': RandomForestRegressor(n_estimators=100, random_state=0),
    ...
    def fit(self,
        ...
        self.clf_.fit(X[:, 2].reshape(-1, 1), y > 0)
    ...
    def predict(self, X: Union[np.ndarray]):
        ...
        return self.clf_.predict(X[:, 2].reshape(-1, 1)) * self.reg_.predict(X)